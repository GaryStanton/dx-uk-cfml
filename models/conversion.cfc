...
--->
	<cffunction
		...
		<cfargument
			...
			required="false"
			default="1"
			hint="Set this to 0 to disable (no headers), or to the row number containing the headers. All rows before that number will be deleted."/>
		...
		<cfif headerRow gt 0>
			<cfset LOCAL.RowHeader = LOCAL.Rows[headerRow] />
			<cfset ArrayDeleteAt(LOCAL.EmptyArray, ArrayLen(LOCAL.EmptyArray)) />
			<cfif headerRow gt 0>
				<cfloop from="1" to="#headerRow#" index="i">
